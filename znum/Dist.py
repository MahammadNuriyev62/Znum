...
class Dist:
    ...
    class Hellinger:
        ...
        def _calculate_H(znum1, znum2):
            ...
            result = min([Dist.Hellinger._formula_hellinger(znum1_column, znum2_column) for znum1_column, znum2_column in zip(znum1_optimization_matrix_transpose, znum2_optimization_matrix_transpose)])
            return result
        ...
        def _calculate_AB(znum1, znum2):
            ...
            for key, (Q1, Q2) in znums.items():
                znum1_half1, znum1_half2, znum2_half1, znum2_half2 = Q1[:halfDimension], reversed(Q1[halfDimension:]), Q2[:halfDimension], reversed(Q2[halfDimension:])
                for znum1_half1_q, znum1_half2_q, znum2_half1_q, znum2_half2_q in zip(znum1_half1, znum1_half2, znum2_half1, znum2_half2):
                    result = Dist.Hellinger._formula_q(znum1_half1_q, znum2_half1_q, znum1_half2_q, znum2_half2_q)
        ...
        def _formula_hellinger(P, Q):
            ...
            """
            H = ((sum([((p ** 0.5) - (q ** 0.5)) ** 2 for p, q in zip(P, Q)])) ** 0.5) / (2 ** 0.5)
            return H
